...
public class Rule0017WriteToFlowField : DiagnosticAnalyzer
{
    ...
    private void AnalyzeAssignmentStatement(OperationAnalysisContext context)
    {
        if (context.IsObsoletePendingOrRemoved())
            return;

        var operation = (IAssignmentStatement)context.Operation;
        ...
        if (fieldSymbol?.FieldClass == FieldClassKind.FlowField && !HasExplainingComment(operation))
            ReportDiagnostic(context, operation.Syntax);
    }
    private IFieldSymbol ExtractFieldSymbol(IOperation argument)
    {
        if (argument is IConversionExpression conversionExpression && conversionExpression.Operand is IFieldAccess fieldAccess)
            return fieldAccess.FieldSymbol;

        return argument is IFieldAccess directFieldAccess ? directFieldAccess.FieldSymbol : null;
    }
    ...
    {
        return operation.Syntax.GetLeadingTrivia().Any(trivia => trivia.IsKind(SyntaxKind.LineCommentTrivia));
    }



    private void ReportDiagnostic(OperationAnalysisContext ctx, SyntaxNode node) =>
        ctx.ReportDiagnostic(
            Diagnostic.Create(DiagnosticDescriptors.Rule0017WriteToFlowField,
            node.GetLocation()));
