...
public class Rule0059SingleQuoteEscaping : DiagnosticAnalyzer
{
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } =
        ImmutableArray.Create(DiagnosticDescriptors.Rule0050OperatorAndPlaceholderInFilterExpression, DiagnosticDescriptors.Rule0059SingleQuoteEscapingIssueDetected, DiagnosticDescriptors.Rule0000ErrorInRule);
        ...
        foreach (var filter in filterExpressions)
        {
            if (filter.ToString().Equals(InvalidUnaryEqualsFilter))
                ctx.ReportDiagnostic(Diagnostic.Create(
                    DiagnosticDescriptors.Rule0059SingleQuoteEscapingIssueDetected,
                    filter.GetLocation()));
        }
    }
}