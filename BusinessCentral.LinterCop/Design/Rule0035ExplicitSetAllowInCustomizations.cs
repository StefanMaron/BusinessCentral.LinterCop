...
{
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } = ImmutableArray.Create(DiagnosticDescriptors.Rule0035ExplicitSetAllowInCustomizations);
    ...
    {
        if (!VersionChecker.IsSupported(ctx.Symbol, Feature.AddPageControlInPageCustomization))
            return;
        ...
        foreach (IFieldSymbol field in fieldsNotReferencedOnPage)
            ctx.ReportDiagnostic(Diagnostic.Create(
                DiagnosticDescriptors.Rule0035ExplicitSetAllowInCustomizations, field.Location!));
    }
        ...
        foreach (IApplicationObjectTypeSymbol relatedPageLike in relatedPages)
        {
            switch (relatedPageLike.GetNavTypeKindSafe())
            {
                case NavTypeKind.Page:
                    IEnumerable<IFieldSymbol> fields = ((IPageTypeSymbol)relatedPageLike).FlattenedControls.Where(x => x.ControlKind == ControlKind.Field && x.RelatedFieldSymbol is not null)
                ...
                case NavTypeKind.PageExtension:
                    IEnumerable<IFieldSymbol> extFields = ((IPageExtensionTypeSymbol)relatedPageLike).AddedControlsFlattened.Where(x => x.ControlKind == ControlKind.Field && x.RelatedFieldSymbol is not null)
                                                        .Select(x => (IFieldSymbol)x.RelatedFieldSymbol!.OriginalDefinition);
    ...
    private static IEnumerable<IApplicationObjectTypeSymbol>? GetRelatedPages(SymbolAnalysisContext ctx)
    {
        ...
        IEnumerable<IApplicationObjectTypeSymbol> pageExtensions = ctx.Compilation.GetDeclaredApplicationObjectSymbols()
                                        ...
                                        .Where(x => ((IApplicationObjectExtensionTypeSymbol)x).Target is not null)
