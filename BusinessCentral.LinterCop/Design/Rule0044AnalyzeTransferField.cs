...
public class Rule0044AnalyzeTransferFields : DiagnosticAnalyzer
{
    ...
    {
        if (ctx.Node is not TableExtensionSyntax tableExtensionSyntax)
            return;
    ...
    private string? GetObjectSourceTable(SyntaxNode node, Compilation compilation)
    {
        switch (node)
        {
            ...
            case PageSyntax pageSyntax:
                ...
                {
                    if (property is not PropertySyntax propertySyntax)
                        continue;

                    if (!propertySyntax.Name.Identifier.ToString().Equals("SourceTable"))
                        continue;

                    if (propertySyntax.Value is not ObjectReferencePropertyValueSyntax objectReferencePropertyValueSyntax)
                        continue;
    ...
    private Dictionary<string, TableExtensionSyntax> GetTableExtensions(Compilation compilation)
    {
        ...
        foreach (SyntaxTree syntaxTree in compilation.SyntaxTrees)
        {
            ...
            {
                if (node is not TableExtensionSyntax tableExtension)
                    continue;
    ...
    private List<VariableDeclarationBaseSyntax> FindVariables(SyntaxNode node, SyntaxKind syntaxKind)
    {
